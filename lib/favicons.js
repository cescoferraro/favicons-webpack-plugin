...
function generateIcons (loader, imageFileStream, pathPrefix, query, callback) {
  ...
  config.appleIcon["meta[name='apple-mobile-web-app-status-bar-style']"] = `<meta name='apple-mobile-web-app-status-bar-style' content='${query.config.appleStatusBarStyle}'>`;

  favicons(imageFileStream, Object.assign({}, query.config, { path: '', url: '' }), function (err, result) {
    if (err) return callback(err);
    var html = result.html.map(function (entry) {
      return entry
        .replace(/(href=[""])/g, '$1' + publicPath + pathPrefix)
        .replace(/(msapplication-(TileImage|config)" content=")/g, '$1' + publicPath + pathPrefix)
      ;
    });
    var loaderResult = {
      prefix: pathPrefix,
